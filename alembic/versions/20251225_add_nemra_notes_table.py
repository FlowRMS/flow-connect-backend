...
def upgrade() -> None:
    ...
    op.create_index(
        "ix_nemra_notes_page_id", "nemra_notes", ["page_id"], schema="report"
    )
    op.create_index(
        "ix_nemra_notes_module_id", "nemra_notes", ["module_id"], schema="report"
    )
    op.create_index(
        "ix_nemra_notes_added_by", "nemra_notes", ["added_by"], schema="report"
    )
    op.create_index(
        "ix_nemra_notes_created_at", "nemra_notes", ["created_at"], schema="report"
    )


def downgrade() -> None:
    op.drop_index(
        "ix_nemra_notes_created_at", table_name="nemra_notes", schema="report"
    )
