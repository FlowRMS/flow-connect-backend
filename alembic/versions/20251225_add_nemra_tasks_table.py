...
def upgrade() -> None:
    ...
    op.create_index(
        "ix_nemra_tasks_page_id", "nemra_tasks", ["page_id"], schema="report"
    )
    op.create_index(
        "ix_nemra_tasks_module_id", "nemra_tasks", ["module_id"], schema="report"
    )
    op.create_index(
        "ix_nemra_tasks_added_by", "nemra_tasks", ["added_by"], schema="report"
    )
    op.create_index(
        "ix_nemra_tasks_assigned_to", "nemra_tasks", ["assigned_to"], schema="report"
    )
    op.create_index(
        "ix_nemra_tasks_due_date", "nemra_tasks", ["due_date"], schema="report"
    )
    op.create_index(
        "ix_nemra_tasks_created_at", "nemra_tasks", ["created_at"], schema="report"
    )


def downgrade() -> None:
    op.drop_index(
        "ix_nemra_tasks_created_at", table_name="nemra_tasks", schema="report"
    )
    op.drop_index("ix_nemra_tasks_due_date", table_name="nemra_tasks", schema="report")
    op.drop_index(
        "ix_nemra_tasks_assigned_to", table_name="nemra_tasks", schema="report"
    )
