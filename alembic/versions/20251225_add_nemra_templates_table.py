...
def upgrade() -> None:
    ...
    op.create_index(
        "ix_nemra_templates_name", "nemra_templates", ["name"], schema="report"
    )
    ...
    _ = op.create_table(
        ...
        sa.ForeignKeyConstraint(
            ["folder_id"], ["report.nemra_folders.id"], ondelete="CASCADE"
        ),
        sa.ForeignKeyConstraint(
            ["template_id"], ["report.nemra_templates.id"], ondelete="CASCADE"
        ),
        sa.UniqueConstraint(
            "folder_id", "template_id", name="uq_nemra_folder_templates_folder_template"
        ),
        schema="report",
    )

    op.create_index(
        "ix_nemra_folder_templates_folder_id",
        "nemra_folder_templates",
        ["folder_id"],
        schema="report",
    )
    op.create_index(
        "ix_nemra_folder_templates_template_id",
        "nemra_folder_templates",
        ["template_id"],
        schema="report",
    )
...
def downgrade() -> None:
    op.drop_index(
        "ix_nemra_folder_templates_template_id",
        table_name="nemra_folder_templates",
        schema="report",
    )
    op.drop_index(
        "ix_nemra_folder_templates_folder_id",
    ...
    op.drop_index(
        "ix_nemra_templates_name", table_name="nemra_templates", schema="report"
    )
