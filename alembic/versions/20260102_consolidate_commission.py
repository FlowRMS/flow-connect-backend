...
def add_column_if_missing(
    table: str, column_name: str, column: sa.Column, schema: str, existing: set[str]
) -> None:
...
def upgrade() -> None:
    ...
    if "order_balances" not in existing_tables:
        _ = op.create_table(
            ...
            sa.Column(
                "quantity", sa.Numeric(18, 6), nullable=False, server_default="0"
            ),
            sa.Column(
                "subtotal", sa.Numeric(18, 6), nullable=False, server_default="0"
            ),
            sa.Column("total", sa.Numeric(18, 6), nullable=False, server_default="0"),
            sa.Column(
                "commission", sa.Numeric(18, 6), nullable=False, server_default="0"
            ),
            sa.Column(
                "discount", sa.Numeric(18, 6), nullable=False, server_default="0"
            ),
            sa.Column(
                "discount_rate", sa.Numeric(18, 6), nullable=False, server_default="0"
            ),
            sa.Column(
                "commission_rate", sa.Numeric(18, 6), nullable=False, server_default="0"
            ),
            sa.Column(
                "commission_discount",
                sa.Numeric(18, 6),
        ...
        existing = {
            col["name"]
            for col in inspector.get_columns("order_balances", schema="pycommission")
        }
        for col_name, col_def in [
            ...
            (
                "discount_rate",
                sa.Column("discount_rate", sa.Numeric(18, 6), nullable=True),
            ),
            (
                "commission_rate",
                sa.Column("commission_rate", sa.Numeric(18, 6), nullable=True),
            ),
            (
                "commission_discount",
                sa.Column("commission_discount", sa.Numeric(18, 6), nullable=True),
            ),
            (
                "commission_discount_rate",
                sa.Column("commission_discount_rate", sa.Numeric(18, 6), nullable=True),
            ),
            (
                "shipping_balance",
                sa.Column("shipping_balance", sa.Numeric(18, 6), nullable=True),
            ),
            (
                "cancelled_balance",
                sa.Column("cancelled_balance", sa.Numeric(18, 6), nullable=True),
            ),
            (
                "freight_charge_balance",
                sa.Column("freight_charge_balance", sa.Numeric(18, 6), nullable=True),
            ),
        ]:
            add_column_if_missing(
                "order_balances", col_name, col_def, "pycommission", existing
            )
    ...
    if "orders" not in existing_tables:
        _ = op.create_table(
            ...
            sa.Column(
                "sold_to_customer_id", postgresql.UUID(as_uuid=True), nullable=False
            ),
            sa.Column(
                "bill_to_customer_id", postgresql.UUID(as_uuid=True), nullable=True
            ),
        ...
        existing = {
            col["name"]
            for col in inspector.get_columns("orders", schema="pycommission")
        }
        for col_name, col_def in [
            (
                "job_id",
    ...
    if "order_details" not in existing_tables:
        _ = op.create_table(
            ...
            sa.Column(
                "unit_price", sa.Numeric(18, 6), nullable=False, server_default="0"
            ),
            sa.Column(
                "subtotal", sa.Numeric(18, 6), nullable=False, server_default="0"
            ),
            ...
            sa.Column(
                "freight_charge", sa.Numeric(18, 6), nullable=False, server_default="0"
            ),
            sa.Column("status", sa.Integer, nullable=False, server_default="1"),
            sa.Column(
                "shipping_balance",
        ...
        op.create_index(
            "ix_pycommission_order_details_order_id",
            "order_details",
            ["order_id"],
            schema="pycommission",
        )
    ...
    if "invoice_balances" not in existing_tables:
        _ = op.create_table(
            ...
            sa.Column(
                "paid_balance", sa.Numeric(18, 6), nullable=False, server_default="0"
            ),
            schema="pycommission",
        )
    ...
    if "invoices" not in existing_tables:
        _ = op.create_table(
            ...
            sa.ForeignKeyConstraint(
                ["balance_id"], ["pycommission.invoice_balances.id"]
            ),
            sa.ForeignKeyConstraint(["created_by_id"], ["public.users.id"]),
            schema="pycommission",
        )
        ...
        existing = {
            col["name"]
            for col in inspector.get_columns("invoices", schema="pycommission")
        }
        for col_name, col_def in [
            (
                "balance_id",
            ...
            add_column_if_missing(
                "invoices", col_name, col_def, "pycommission", existing
            )
    ...
    if "invoice_details" not in existing_tables:
        ...
        op.create_index(
            "ix_pycommission_invoice_details_invoice_id",
            "invoice_details",
            ["invoice_id"],
            schema="pycommission",
        )
        op.create_index(
            "ix_pycommission_invoice_details_order_detail_id",
            "invoice_details",
            ["order_detail_id"],
            schema="pycommission",
        )
    ...
    if "order_split_rates" not in existing_tables:
        ...
        op.create_index(
            "ix_pycommission_order_split_rates_order_detail_id",
            "order_split_rates",
            ["order_detail_id"],
            schema="pycommission",
        )
    ...
    if "order_inside_reps" not in existing_tables:
        ...
        op.create_index(
            "ix_pycommission_order_inside_reps_order_detail_id",
            "order_inside_reps",
            ["order_detail_id"],
            schema="pycommission",
        )
    ...
    if "invoice_split_rates" not in existing_tables:
        _ = op.create_table(
            ...
            sa.Column("id", postgresql.UUID(as_uuid=True), primary_key=True),
            sa.Column(
                "invoice_detail_id", postgresql.UUID(as_uuid=True), nullable=False
            ),
            sa.Column("user_id", postgresql.UUID(as_uuid=True), nullable=False),
            sa.Column("split_rate", sa.Numeric(5, 4), nullable=False),
            sa.Column("position", sa.Integer, nullable=False),
        ...
        op.create_index(
            "ix_pycommission_invoice_split_rates_invoice_detail_id",
            "invoice_split_rates",
            ["invoice_detail_id"],
            schema="pycommission",
        )
