...
def upgrade() -> None:
    ...
    columns = [
        col["name"] for col in inspector.get_columns("factories", schema="pycore")
    ]

    # Add overage_allowed boolean field
    if "overage_allowed" not in columns:
        op.add_column(
            "factories",
            sa.Column(
                "overage_allowed",
    ...
    if "overage_type" not in columns:
        op.add_column(
            "factories",
            sa.Column(
                "overage_type",
    ...
    if "rep_overage_share" not in columns:
        op.add_column(
            "factories",
            sa.Column(
                "rep_overage_share",
