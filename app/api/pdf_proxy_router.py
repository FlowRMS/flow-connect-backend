"""PDF Proxy Router - Proxies PDF requests to avoid CORS issues."""
...
async def pdf_proxy(url: str = Query(..., description="URL of the PDF to proxy")):  # pyright: ignore[reportCallInDefaultInitializer]
    ...
    This avoids CORS issues when trying to load PDFs from S3 or external domains.
    """
    if not url:
    ...
    try:
        ...
        raise HTTPException(status_code=504, detail="Request to PDF timed out")
