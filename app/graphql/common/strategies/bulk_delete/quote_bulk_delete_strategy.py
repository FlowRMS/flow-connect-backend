...
class QuoteBulkDeleteStrategy(BulkDeleteStrategy):
    def __init__(
        self,
    ...
    def get_supported_entity_type(self) -> BulkDeleteEntityType:
        return BulkDeleteEntityType.QUOTES
    ...
    async def delete_entities(self, entity_ids: list[UUID]) -> BulkDeleteResult:
        ...
        for entity_id in entity_ids:
            async with self.session.begin_nested():
                try:
                    _ = await self.quote_service.delete_quote(entity_id)
                    deleted_count += 1
                except NotFoundError:
                    failures.append(
                        BulkDeleteFailure(
                            entity_id=entity_id,
                            error=f"Quote with id {entity_id} not found",
                        )
                    )
        ...
        if failures:
            return BulkDeleteResult.partial(
                entity_type=BulkDeleteEntityType.QUOTES,
