...
class CheckRelatedEntitiesStrategy(RelatedEntitiesStrategy):
    def __init__(
        self,
    ...
    def get_supported_source_type(self) -> LandingSourceType:
        return LandingSourceType.CHECKS
    ...
    async def get_related_entities(self, entity_id: UUID) -> RelatedEntitiesResponse:
        ...
        jobs = await self.jobs_service.find_by_entity(EntityType.CHECK, entity_id)
        quotes = await self.quote_service.find_by_entity(EntityType.CHECK, entity_id)
        orders = await self.order_service.find_by_entity(EntityType.CHECK, entity_id)
        ...
        return RelatedEntitiesResponse(
            source_type=LandingSourceType.CHECKS,
