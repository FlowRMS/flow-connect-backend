...
class FileRelatedEntitiesStrategy(RelatedEntitiesStrategy):
    def __init__(
        self,
    ...
    def get_supported_source_type(self) -> LandingSourceType:
        return LandingSourceType.FILES
    ...
    async def get_related_entities(self, entity_id: UUID) -> RelatedEntitiesResponse:
        ...
        jobs = await self.jobs_service.find_by_entity(EntityType.FILE, entity_id)
        quotes = await self.quote_service.find_by_entity(EntityType.FILE, entity_id)
        orders = await self.order_service.find_by_entity(EntityType.FILE, entity_id)
        invoices = await self.invoice_service.find_by_entity(EntityType.FILE, entity_id)
        checks = await self.check_service.find_by_entity(EntityType.FILE, entity_id)
        factories = await self.factory_service.find_by_entity(
            EntityType.FILE, entity_id
        )
        products = await self.product_service.find_by_entity(EntityType.FILE, entity_id)
