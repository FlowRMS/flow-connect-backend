...
class JobRelatedEntitiesStrategy(RelatedEntitiesStrategy):
    def __init__(
        self,
    ...
    async def get_related_entities(self, entity_id: UUID) -> RelatedEntitiesResponse:
        ...
        contacts = await self.contacts_service.find_contacts_by_job_id(entity_id)
        companies = await self.companies_service.find_companies_by_job_id(entity_id)
        quotes = await self.quote_service.find_quotes_by_job_id(entity_id)
        orders = await self.order_service.find_orders_by_job_id(entity_id)
        invoices = await self.invoice_service.find_invoices_by_job_id(entity_id)
        checks = await self.check_service.find_checks_by_job_id(entity_id)
        factories = await self.factory_service.find_by_entity(EntityType.JOB, entity_id)
        products = await self.product_service.find_by_entity(EntityType.JOB, entity_id)
        customers = await self.customer_service.find_by_entity(
            EntityType.JOB, entity_id
        )
