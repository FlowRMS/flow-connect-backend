...
class OrderRelatedEntitiesStrategy(RelatedEntitiesStrategy):
    def __init__(
        self,
    ...
    def get_supported_source_type(self) -> LandingSourceType:
        return LandingSourceType.ORDERS
    ...
    async def get_related_entities(self, entity_id: UUID) -> RelatedEntitiesResponse:
        ...
        jobs = await self.jobs_service.find_by_entity(EntityType.ORDER, entity_id)
        quotes = await self.quote_service.find_by_entity(EntityType.ORDER, entity_id)
        invoices = await self.invoice_service.find_invoices_by_order_id(entity_id)
        checks = await self.check_service.find_by_entity(EntityType.ORDER, entity_id)
        ...
        return RelatedEntitiesResponse(
            source_type=LandingSourceType.ORDERS,
