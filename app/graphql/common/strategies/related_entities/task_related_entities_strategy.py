...
class TaskRelatedEntitiesStrategy(RelatedEntitiesStrategy):
    def __init__(
        self,
    ...
    def get_supported_source_type(self) -> LandingSourceType:
        return LandingSourceType.TASKS
    ...
    async def get_related_entities(self, entity_id: UUID) -> RelatedEntitiesResponse:
        ...
        jobs = await self.jobs_service.get_jobs_by_task(entity_id)
        notes = await self.notes_service.find_notes_by_entity(
            EntityType.TASK, entity_id
        )
        contacts = await self.contacts_service.find_contacts_by_task_id(entity_id)
        companies = await self.companies_service.find_companies_by_task_id(entity_id)
        pre_opportunities = await self.pre_opportunities_service.find_by_entity(
            EntityType.TASK, entity_id
        )
        quotes = await self.quote_service.find_by_entity(EntityType.TASK, entity_id)
        orders = await self.order_service.find_by_entity(EntityType.TASK, entity_id)
        invoices = await self.invoice_service.find_by_entity(EntityType.TASK, entity_id)
        checks = await self.check_service.find_by_entity(EntityType.TASK, entity_id)
        factories = await self.factory_service.find_by_entity(
            EntityType.TASK, entity_id
        )
        products = await self.product_service.find_by_entity(EntityType.TASK, entity_id)
        customers = await self.customer_service.find_by_entity(
            EntityType.TASK, entity_id
        )

        return RelatedEntitiesResponse(
            source_type=LandingSourceType.TASKS,
