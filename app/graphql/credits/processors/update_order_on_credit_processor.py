...
from commons.db.v6.commission import Credit, CreditDetail, InvoiceDetail
...
class UpdateOrderOnCreditProcessor(BaseProcessor[Credit]):
    ...
    async def process(self, context: EntityContext[Credit]) -> None:
        ...
        order = await self._get_order_with_details(credit.order_id)
        if order is None:
            return
    ...
    async def _get_credited_amount_for_detail(self, order_detail_id: UUID) -> Decimal:
        result = await self.session.execute(
            select(func.coalesce(func.sum(CreditDetail.total), Decimal("0"))).where(
                CreditDetail.order_detail_id == order_detail_id
            )
        )
        return result.scalar_one()
    ...
    async def _update_order_detail_statuses(self, order: Order) -> None:
        for detail in order.details:
            invoiced = await self._get_invoiced_amount_for_detail(detail.id)
            credited = await self._get_credited_amount_for_detail(detail.id)
            net_invoiced = invoiced - credited
            new_status = self._calculate_detail_status(detail.total, net_invoiced)
            detail.status = new_status

    def _calculate_detail_status(
        self, order_total: Decimal, net_invoiced: Decimal
    ) -> OrderStatus:
        if net_invoiced <= Decimal("0"):
            return OrderStatus.OPEN
        if net_invoiced < order_total:
