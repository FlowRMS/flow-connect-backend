...
from commons.db.v6.commission import Invoice, InvoiceSplitRate
...
class InvoiceDefaultRepSplitProcessor(BaseProcessor[Invoice]):
    ...
    async def process(self, context: EntityContext[Invoice]) -> None:
        ...
        order_detail_ids = self._collect_order_detail_ids(invoice)

        if not order_detail_ids:
            return

        order_details_map = await self._load_order_details(order_detail_ids)
        self._apply_outside_reps_from_order_details(invoice, order_details_map)

    def _collect_order_detail_ids(self, invoice: Invoice) -> list[UUID]:
        return [
