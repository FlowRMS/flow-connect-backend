...
class OrderAcknowledgementService:
    ...
    async def create(self, input: OrderAcknowledgementInput) -> OrderAcknowledgement:
        ...
        order_detail_ids = [detail.order_detail_id for detail in input.details]
        await self.repository.sync_detail_links(created.id, order_detail_ids)
    ...
    async def update(self, input: OrderAcknowledgementInput) -> OrderAcknowledgement:
        ...
        order_detail_ids = [detail.order_detail_id for detail in input.details]
        await self.repository.sync_detail_links(updated.id, order_detail_ids)
