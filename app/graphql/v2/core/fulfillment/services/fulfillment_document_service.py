...
class FulfillmentDocumentService:
    ...
    async def delete_document(self, document_id: UUID) -> bool:
        ...
        if document.file_id:
            await self.file_service.delete_file(document.file_id)
        elif document.file_url:
            # Fallback for documents without linked File record
            s3_key = self.file_upload_service.extract_s3_key_from_path(document.file_url)
            if s3_key:
    ...
    async def upload_document(
        ...
        presigned_url = await self.file_upload_service.get_presigned_url(
            self.file_upload_service.extract_s3_key_from_path(file_record.file_path)
        )
