...
class GmailTokenRepository(BaseRepository[GmailUserToken]):
    """Repository for Gmail user token CRUD operations."""
    ...
    def __init__(
        ...
        super().__init__(session, context_wrapper, GmailUserToken)

    async def get_by_user_id(self, user_id: uuid.UUID) -> GmailUserToken | None:
        """
        ...
        result = await self.session.execute(
            select(GmailUserToken).where(GmailUserToken.user_id == user_id)
        )
        return result.scalar_one_or_none()

    async def get_active_token(self, user_id: uuid.UUID) -> GmailUserToken | None:
        """
    ...
    async def deactivate_token(self, user_id: uuid.UUID) -> bool:
        ...
        result = await self.session.execute(
            ...
            .where(GmailUserToken.user_id == user_id)
    ...
    async def update_tokens(
        ...
        result = await self.session.execute(
            select(GmailUserToken).where(GmailUserToken.id == token_id)
        )
